...
async function installFromNpm(packageName, registry) {
  ...
  try {
    ...
    // Create temp directory for download with cryptographically secure random name
    // Prevents race condition attacks where an attacker predicts the temp directory name
    ...
    try {
      ...
      // Extract using execFileSync to prevent command injection
      // tarballPath and tempDir are generated internally, but we use array args for defense in depth
      ...
      if (manifest.dependencies && manifest.dependencies.length > 0) {
        ...
        for (const dep of manifest.dependencies) {
          try {
            // Use execFileSync with array args to prevent command injection via dependency names
      ...
      // Securely wipe temp directory before removal (may contain sensitive data during plugin installation)
      const { secureWipeDirectory } = require('./security');
      await secureWipeDirectory(tempDir).catch(() => {});
    }
...
async function installFromGit(gitUrl, registry) {
  ...
  try {
    ...
    // Clone repository using execFileSync to prevent command injection via gitUrl
