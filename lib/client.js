...
const { checkRateLimit, RateLimitCategory } = require('./rate-limiter');
 ...
 * Uses secure HTTP client with SSRF protection
 */
async function fetchOpenApiSpec(coreUrl) {
  try {
    const url = `${coreUrl}/openapi.json`;
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ECONNRESET') {
      const err = new Error(`Core API not available at ${coreUrl}. Is MasterClaw running?`);
      err.code = 'SERVICE_UNAVAILABLE';
      throw err;
    }
    if (error.code === 'SSRF_VIOLATION') {
      const err = new Error(`Security violation: ${error.message}`);
      err.code = 'SSRF_VIOLATION';
      throw err;
    }
    if (error.code === 'RESPONSE_TOO_LARGE') {
      const err = new Error('OpenAPI spec response too large');
...
client
  ...
  .action(wrapCommand(async (options) => {
    ...
    if (!rateLimitCheck.allowed) {
      const err = new Error(`Rate limit exceeded. Try again in ${rateLimitCheck.retryAfterSec} seconds.`);
    ...
    if (!validLangs.includes(options.lang)) {
      const err = new Error(`Unsupported language: ${options.lang}. Supported: ${validLangs.join(', ')}`);
    ...
    try {
      ...
      const err = new Error(`Invalid URL format: ${options.url}`);
      err.code = 'INVALID_ARGUMENTS';
      throw err;
    }
...
client
  ...
  .action(wrapCommand(async (options) => {
    // Check rate limit before processing
    const rateLimitCheck = checkRateLimit('client-validate', RateLimitCategory.READ_ONLY);
    if (!rateLimitCheck.allowed) {
      const err = new Error(`Rate limit exceeded. Try again in ${rateLimitCheck.retryAfterSec} seconds.`);
      err.code = 'RATE_LIMIT_EXCEEDED';
      throw err;
...
client
  ...
  .action(wrapCommand(async (options) => {
    // Check rate limit before processing
    const rateLimitCheck = checkRateLimit('client-diff', RateLimitCategory.READ_ONLY);
    if (!rateLimitCheck.allowed) {
      const err = new Error(`Rate limit exceeded. Try again in ${rateLimitCheck.retryAfterSec} seconds.`);
      err.code = 'RATE_LIMIT_EXCEEDED';
      throw err;
    ...
    const validLangs = ['typescript', 'javascript', 'python'];
    if (!validLangs.includes(options.lang)) {
      const err = new Error(`Unsupported language: ${options.lang}`);
      err.code = 'INVALID_ARGUMENTS';
      throw err;
    }
...
client
  ...
  .action(wrapCommand(async (options) => {
    ...
    // Check rate limit before processing
    const rateLimitCheck = checkRateLimit('client-sync', RateLimitCategory.DATA_MODIFICATION);
    if (!rateLimitCheck.allowed) {
      const err = new Error(`Rate limit exceeded. Try again in ${rateLimitCheck.retryAfterSec} seconds.`);
      err.code = 'RATE_LIMIT_EXCEEDED';
      throw err;
    ...
    const validLangs = ['typescript', 'javascript', 'python'];
    const invalidLangs = options.lang.filter(l => !validLangs.includes(l));
    if (invalidLangs.length > 0) {
      const err = new Error(`Unsupported languages: ${invalidLangs.join(', ')}`);
      err.code = 'INVALID_ARGUMENTS';
      throw err;
    }
