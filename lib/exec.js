 ...
 * - Resource limits (fork bomb protection, memory limits)
 */
...
/** Docker command timeout for status checks (10 seconds) */
const DOCKER_STATUS_TIMEOUT_MS = 10 * 1000;

// =============================================================================
// Resource Limit Configuration (Security Hardening)
// =============================================================================

/**
 * Resource limits for container execution to prevent fork bombs and
 * resource exhaustion attacks. These limits are applied via docker exec
 * --ulimit flags to restrict what commands can consume.
 *
 * Security benefits:
 * - Prevents fork bombs (max PIDs limit)
 * - Limits memory consumption (max memory)
 * - Prevents CPU starvation (CPU shares)
 * - Restricts file descriptor exhaustion
 */
const RESOURCE_LIMITS = {
  ...
  nproc: {
    soft: 128,
    hard: 256,
    description: 'Maximum processes (fork bomb protection)',
  },
  ...
  /** Maximum memory (512MB soft, 1GB hard) - prevents memory exhaustion */
  memory: {
    soft: 536870912,   // 512MB in bytes
    hard: 1073741824,  // 1GB in bytes
    description: 'Maximum memory (512MB soft, 1GB hard)',
  },
  /** Maximum stack size (8MB) - prevents stack overflow attacks */
  stack: {
    soft: 8388608,     // 8MB in bytes
    hard: 16777216,    // 16MB in bytes
    description: 'Maximum stack size',
  },
};

/** Environment variable to disable resource limits (emergency override) */
const DISABLE_RESOURCE_LIMITS_ENV = 'MC_EXEC_NO_RESOURCE_LIMITS';

// =============================================================================
// Exit Code Analysis - Resource Limit Detection
// =============================================================================

/**
 * Exit code meanings for resource limit violations
 * These codes indicate when Docker/linux killed a process due to resource limits
 */
const EXIT_CODES = {
  /** SIGKILL (9) - Often sent by OOM killer or manual kill */
  SIGKILL: 137,  // 128 + 9
  ...
  SIGXFSZ: 153,  // 128 + 25
  /** SIGSYS (31) - Bad system call (seccomp violation) */
  SIGSYS: 159,   // 128 + 31
  /** Container OOM - Special Docker exit code */
  CONTAINER_OOM: 1,  // Often accompanied by OOM message
};

/**
 * Analyzes exit code to determine if resource limits were violated
 * @param {number} exitCode - Process exit code
 * @returns {Object} - Analysis result with violation details
 */
function analyzeExitCode(exitCode) {
  const result = {
    exitCode,
    isResourceViolation: false,
    violationType: null,
    description: null,
    suggestion: null,
  };
  ...
  switch (exitCode) {
    case EXIT_CODES.SIGKILL:
      result.isResourceViolation = true;
      result.violationType = 'RESOURCE_LIMIT';
      result.description = 'Process was forcefully killed (likely OOM or nproc limit)';
      result.suggestion = 'Command may have exceeded memory (1GB) or process (256) limits. Try simplifying the command or use --shell for interactive debugging.';
      break;
    case EXIT_CODES.SIGXCPU:
      result.isResourceViolation = true;
      result.violationType = 'CPU_LIMIT';
      result.description = 'CPU time limit exceeded';
      result.suggestion = 'Command consumed too much CPU time. Consider optimizing the command or running outside container.';
      break;
    case EXIT_CODES.SIGXFSZ:
      result.isResourceViolation = true;
      result.violationType = 'FILE_SIZE_LIMIT';
      result.description = 'File size limit exceeded';
      result.suggestion = 'Command attempted to create a file larger than allowed.';
      break;
    case EXIT_CODES.SIGSYS:
      result.isResourceViolation = true;
      result.violationType = 'SYSTEM_CALL_BLOCKED';
      result.description = 'Blocked system call (seccomp violation)';
      result.suggestion = 'Command attempted a forbidden system call. This may indicate incompatible software.';
      break;
    default:
      if (exitCode !== 0) {
        result.description = `Command exited with code ${exitCode}`;
      }
      break;
  }
...
/**
 * Checks stderr for OOM (Out of Memory) indicators
 * @param {string} stderr - Standard error output
 * @returns {boolean} - True if OOM was detected
 */
function detectOOMFromStderr(stderr) {
  ...
  return oomIndicators.some(pattern => pattern.test(stderr));
}
 ...
 */
async function isContainerRunning(container, timeout = DOCKER_STATUS_TIMEOUT_MS) {
...
async function execInContainer(options) {
  ...
  // Add resource limits to prevent fork bombs and resource exhaustion
  // Skip if explicitly disabled via environment variable (emergency override)
  if (!process.env[DISABLE_RESOURCE_LIMITS_ENV]) {
    // Maximum number of processes (fork bomb protection)
    dockerArgs.push('--ulimit', `nproc=${RESOURCE_LIMITS.nproc.soft}:${RESOURCE_LIMITS.nproc.hard}`);
    // Maximum open file descriptors
    dockerArgs.push('--ulimit', `nofile=${RESOURCE_LIMITS.nofile.soft}:${RESOURCE_LIMITS.nofile.hard}`);
    // Maximum memory (using --memory flag for cgroups enforcement)
    dockerArgs.push('--memory', String(RESOURCE_LIMITS.memory.hard));
    dockerArgs.push('--memory-swap', String(RESOURCE_LIMITS.memory.hard));
    // Maximum stack size
    dockerArgs.push('--ulimit', `stack=${RESOURCE_LIMITS.stack.soft}:${RESOURCE_LIMITS.stack.hard}`);
  }

  dockerArgs.push(container, ...command);

  // Log audit event
  await logAudit(AuditEventType.DOCKER_EXEC, {
    container,
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', async (code) => {
      ...
      // Analyze exit code for resource limit violations
      const exitAnalysis = analyzeExitCode(code);
      const hasOOM = detectOOMFromStderr(stderr);

      // If OOM detected in stderr but exit code doesn't indicate it, update analysis
      if (hasOOM && !exitAnalysis.isResourceViolation) {
        exitAnalysis.isResourceViolation = true;
        exitAnalysis.violationType = 'MEMORY_LIMIT';
        exitAnalysis.description = 'Out of Memory (OOM) - Container exceeded memory limit';
        exitAnalysis.suggestion = `Command exceeded the ${Math.round(RESOURCE_LIMITS.memory.hard / 1024 / 1024)}MB memory limit. Try reducing memory usage or run the command with MC_EXEC_NO_RESOURCE_LIMITS=1 (emergency override).`;
      }

      // Log resource limit violations to audit
      if (exitAnalysis.isResourceViolation) {
        await logAudit(AuditEventType.SECURITY_VIOLATION, {
          container,
          command: command.join(' '),
          violationType: exitAnalysis.violationType,
          exitCode: code,
          description: exitAnalysis.description,
          stderr: stderr?.substring(0, 500), // Limit stderr in audit log
        });
      }
      ...
      if (interactive) {
        resolve({
          ...
          duration,
          interactive: true,
          resourceViolation: exitAnalysis.isResourceViolation ? exitAnalysis : null,
        });
      } else {
        resolve({
          success: code === 0,
 ...
 */
async function getRunningContainers(timeout = DOCKER_STATUS_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    const containers = [];
