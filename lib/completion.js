...
const COMMANDS = [
  ...
  'audit', 'audit-verify', 'context', 'client', 'api'
];

// Command-specific arguments
const COMMAND_ARGS = {
  'logs': SERVICES,
  'api': ['status', 'docs', 'export', 'endpoints', 'version', '--url', '-u', '--json', '--redoc', '--yaml', '--output', '-o', '--tag', '-t'],
  'backup': ['list', 'stats', 'cleanup', 'export', '--retention-days', '--force', '--dry-run', '-r', '-f', '-d'],
  'client': ['generate', 'validate', 'diff', 'sync', 'status', '--lang', '--url', '--output', '-l', '-u', '-o', '--typescript', '--javascript', '--python'],
...
_mc_completion() {
    ...
    case "\${prev}" in
        ...
        completion)
            ...
            ;;
        client)
            local client_cmds="generate validate diff sync status"
            local client_opts="--lang --url --output -l -u -o --skip-package --json --commit"
            COMPREPLY=( $(compgen -W "\${client_cmds} \${client_opts}" -- \${cur}) )
            return 0
            ;;
        api)
            local api_cmds="status docs export endpoints version"
            local api_opts="--url -u --json --redoc --yaml --output -o --tag -t"
            COMPREPLY=( $(compgen -W "\${api_cmds} \${api_opts}" -- \${cur}) )
            return 0
            ;;
        -i|--infra-dir)
    ...
    case "$state" in
        args)
            case "$line[1]" in
                ...
                client)
                    _arguments \
                        '(-l --lang)'{-l,--lang}'[Target language]:language:(typescript javascript python)' \
                        '(-u --url)'{-u,--url}'[Core API URL]:url:' \
                        '(-o --output)'{-o,--output}'[Output directory]:directory:_directories' \
                        '--skip-package[Skip generating package files]' \
                        '--json[Output as JSON]' \
                        '--commit[Auto-commit changes]' \
                        '1: :_mc_client_commands'
                    ;;
            esac
            ;;
    esac
...
_mc_client_commands() {
    local commands=(
        ...
        'validate:Validate Core API OpenAPI spec'
        'diff:Show API changes since last generation'
        'sync:Auto-sync clients when API changes'
        'status:Show client generation status'
    )
    _describe -t commands 'client commands' commands
}
...
function getCommandDescription(cmd) {
  const descriptions = {
    ...
    'client': 'Generate API clients from OpenAPI spec',
  };
...
# Client subcommands
complete -c mc -n "__fish_seen_subcommand_from client" -a "generate" -d "Generate API client from OpenAPI spec"
complete -c mc -n "__fish_seen_subcommand_from client" -a "validate" -d "Validate Core API OpenAPI spec"
complete -c mc -n "__fish_seen_subcommand_from client" -a "diff" -d "Show API changes since last generation"
complete -c mc -n "__fish_seen_subcommand_from client" -a "sync" -d "Auto-sync clients when API changes"
complete -c mc -n "__fish_seen_subcommand_from client" -a "status" -d "Show client generation status"
complete -c mc -n "__fish_seen_subcommand_from client" -s l -l lang -d "Target language (typescript, javascript, python)" -r
complete -c mc -n "__fish_seen_subcommand_from client" -s u -l url -d "Core API URL" -r
complete -c mc -n "__fish_seen_subcommand_from client" -s o -l output -d "Output directory" -r
complete -c mc -n "__fish_seen_subcommand_from client" -l skip-package -d "Skip generating package files"
complete -c mc -n "__fish_seen_subcommand_from client" -l json -d "Output as JSON"
complete -c mc -n "__fish_seen_subcommand_from client" -l commit -d "Auto-commit changes"
`;
}
