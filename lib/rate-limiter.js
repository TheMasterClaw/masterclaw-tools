...
function isValidRateLimitState(state) {
  ...
  for (const [command, entries] of Object.entries(state)) {
    // Command name must be a safe string
  ...
  try {
    if (await fs.pathExists(RATE_LIMIT_FILE)) {
      ...
      if (isValidRateLimitState(state)) {
        ...
        // Log security event if corruption looks suspicious
        if (state && typeof state === 'object' && state.__proto__) {
          await logSecurityViolation('RATE_LIMIT_STATE_CORRUPTION', {
