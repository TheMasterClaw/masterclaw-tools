...
/** Dangerous keys that could enable prototype pollution attacks */
...
/**
 * Sanitizes a key to prevent prototype pollution
...
function sanitizeKey(key) {
  if (typeof key !== 'string') {
    throw new Error(`Config key must be a string, got ${typeof key}`);
  }
  
  if (isDangerousKey(key)) {
    throw new Error(`Config key '${key}' is not allowed (prototype pollution protection)`);
  }
...
/**
 * Safely merges objects without prototype pollution
...
/**
 * Sanitizes an entire config object by removing dangerous keys recursively
...
function sanitizeConfigObject(obj) {
  ...
  if (Array.isArray(obj)) {
    return obj.map(sanitizeConfigObject);
  }
...
// Load config with prototype pollution protection
async function loadConfig() {
  await ensureConfigDir();
  
  if (await fs.pathExists(CONFIG_FILE)) {
    const rawConfig = await fs.readJson(CONFIG_FILE);
    // Sanitize the loaded config to remove any dangerous keys
    const sanitizedConfig = sanitizeConfigObject(rawConfig);
    // Use safe deep merge to prevent prototype pollution
    return safeDeepMerge({ ...DEFAULT_CONFIG }, sanitizedConfig);
  }
...
// Set a config value with prototype pollution protection
async function set(key, value) {
  ...
  // Sanitize the final key before setting
  const finalKey = sanitizeKey(keys[keys.length - 1]);
