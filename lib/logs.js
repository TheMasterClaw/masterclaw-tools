...
function validateServiceName(service) {
  if (typeof service !== 'string') {
    throw new Error(`Service name must be a string, got ${typeof service}`);
  }

  if (!VALID_SERVICES.has(service)) {
    throw new Error(`Unknown service: ${service}. Valid services: ${Array.from(VALID_SERVICES).join(', ')}`);
  }
...
function validateDuration(duration) {
  if (typeof duration !== 'string') {
    throw new Error(`Duration must be a string, got ${typeof duration}`);
  }

  if (!VALID_DURATION.test(duration)) {
    throw new Error(`Invalid duration format: ${duration}. Use format like: 30s, 5m, 1h, 24h, 7d, 2w`);
  }
  ...
  if (value > maxValues[unit]) {
    throw new Error(`Duration too large: ${duration}. Maximum is ${maxValues[unit]}${unit}`);
  }
...
function validateLines(lines, maxLines = MAX_EXPORT_LINES) {
  ...
  if (isNaN(numLines) || !Number.isInteger(numLines)) {
    throw new Error(`Lines must be an integer, got: ${lines}`);
  }

  if (numLines < 0) {
    throw new Error(`Lines cannot be negative: ${numLines}`);
  }

  if (numLines > maxLines) {
    throw new Error(`Lines exceeds maximum of ${maxLines}: ${numLines}`);
  }
...
function validateSearchPattern(pattern) {
  ...
  if (DANGEROUS_CHARS.test(pattern)) {
    throw new Error('Search pattern contains potentially dangerous characters');
  }

  // Check for null bytes
  if (pattern.includes('\0')) {
    throw new Error('Search pattern cannot contain null bytes');
  }
...
function validateOutputDir(dirPath) {
  if (typeof dirPath !== 'string') {
    throw new Error(`Output directory must be a string, got ${typeof dirPath}`);
  }
  ...
  if (dirPath.includes('\0')) {
    throw new Error('Output directory cannot contain null bytes');
  }

  // Normalize and check for path traversal
  const normalized = path.normalize(dirPath);
  if (normalized.startsWith('..') || normalized.includes('../') || normalized.includes('..\\')) {
    throw new Error('Output directory cannot contain path traversal sequences');
  }
