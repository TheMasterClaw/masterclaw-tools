...
function generateEnvTemplate(options) {
  const opts = options || {};
  const domain = opts.domain || 'mc.example.com';
...
OPENAI_API_KEY=${opts.openaiKey || 'sk-...'}
...
acme_email = "${opts.email || 'admin@' + domain}"
...
# AWS Region
aws_region = "${opts.awsRegion || 'us-east-1'}"
...
# EKS cluster name
cluster_name = "masterclaw-${opts.environment || 'dev'}"
...
use_spot_instances = ${opts.environment === 'dev' ? 'true' : 'false'}
...
# RDS instance class
db_instance_class = "${opts.environment === 'prod' ? 'db.t3.medium' : 'db.t3.micro'}"

# Database username
db_username = "masterclaw"

# Enable Multi-AZ for production
db_multi_az = ${opts.environment === 'prod' ? 'true' : 'false'}
...
function generateServiceTemplate(options) {
  const opts = options || {};
  const serviceName = opts.serviceName || 'my-service';
