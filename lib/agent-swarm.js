...
// Ensure metering is initialized
let meteringInstance = null;
function getMetering() {
  if (!meteringInstance) {
    meteringInstance = metering.getMetering();
    // Initialize without autoFlush for tests
    meteringInstance.init().catch(() => {});
  }
...
class Swarm extends EventEmitter {
  ...
  async run(options) {
    ...
    getMetering().recordEvent('swarm.task.started', {
      taskId,
      agentId: agent.id,
    ...
    try {
      ...
      while (task.turnCount < maxTurns) {
        ...
        getMetering().recordEvent('swarm.task.completed', {
          taskId,
          duration,
      ...
      getMetering().recordEvent('swarm.task.failed', {
        taskId,
        duration,
        error: error.message,
      });
