...
const { getCurrentCorrelationId, createChildCorrelationId, runWithCorrelationIdAsync } = require('./correlation');
const { flush: flushLogger } = require('./logger');
 ...
 * JSON mode is useful for production environments and CI/CD pipelines
 * where structured log aggregation is required.
 *
 * Enable with: MC_JSON_OUTPUT=1 mc <command>
 *
 * @returns {boolean} - True if JSON output is enabled
 */
function isJsonOutputMode() {
  // Check environment variable
  if (process.env.MC_JSON_OUTPUT === '1' || process.env.MC_JSON_OUTPUT === 'true') {
    return true;
  }
  return false;
}
...
const ERROR_MESSAGE_MAP = [
  ...
  {
    pattern: /docker.*timed out|timeout.*docker|COMPOSE_TIMEOUT|LOGS_TIMEOUT/i,
    category: ErrorCategory.DOCKER,
    message: 'Docker command timed out.',
    suggestion: 'Docker daemon may be unresponsive. Try restarting Docker or check system resources.',
    exitCode: ExitCode.DOCKER_ERROR,
  },
  {
 ...
 * Supports both human-readable and JSON output modes for production/CI environments.
 *
 * JSON mode (MC_JSON_OUTPUT=1) outputs structured logs suitable for log aggregation:
 * {
 *   "level": "error",
...
function displayError(err, options = {}) {
  ...
  if (jsonMode) {
    // Structured JSON output for production/CI environments
    const errorOutput = {
      level: classification.category === ErrorCategory.SECURITY ? 'error' : 'error',
      ...
      error: {
        type: err.name || 'Error',
        message: maskSensitiveData(err.message || 'Unknown error'),
    ...
    if (suggestion) {
      errorOutput.suggestion = suggestion;
    }
    ...
    // Print correlation ID for error tracking (in verbose mode or if exists)
    if (correlationId && (verbose || classification.category === ErrorCategory.SECURITY)) {
...
function wrapCommand(handler, commandName = 'unknown') {
  return async (...args) => {
    // Create a child correlation ID for this command execution
    const commandCorrelationId = createChildCorrelationId();
    ...
    return runWithCorrelationIdAsync(async () => {
      try {
        ...
        // Display error to user with correlation ID
        const exitCode = displayError(err, {
          verbose,
          command: commandName,
        });

        // Log to audit system (fire and forget) with correlation ID
 ...
 * Supports JSON output mode for production environments.
 */
  ...
  process.on('unhandledRejection', (reason, promise) => {
    if (jsonMode) {
      console.error(JSON.stringify({
        ...
        message: reason instanceof Error ? maskSensitiveData(reason.message) : String(reason),
        error: reason instanceof Error ? {
          type: reason.name,
          message: maskSensitiveData(reason.message),
          stack: process.env.MC_VERBOSE ? maskSensitiveData(reason.stack) : undefined,
        } : { value: String(reason) },
      }));
    } else {
    ...
    // Flush logs before exit to ensure critical events are persisted
    flushLogger().finally(() => {
    ...
    if (jsonMode) {
      console.error(JSON.stringify({
        ...
        error: {
          type: err.name,
          message: maskSensitiveData(err.message),
          stack: process.env.MC_VERBOSE ? maskSensitiveData(err.stack) : undefined,
        },
      }));
    } else {
...
module.exports = {
  ...
  getUserMessage,
  getSuggestion,
  getRemediationSteps: getSuggestion, // Alias for backward compatibility
