...
class CircuitBreaker {
  ...
  constructor(name, config = {}) {
    ...
    this.stats = {
      ...
      totalSuccesses: 0,
      totalFailures: 0,
      totalFallbacks: 0,  // Track fallback usage for resilience metrics
      stateTransitions: [],
      lastFailureTime: null,
      lastSuccessTime: null,
      lastFallbackTime: null,  // Track when fallback was last used
      openedAt: null,
      closedAt: Date.now(),
    };
  ...
  recordSuccess() {
    ...
    if (this.state === CircuitState.HALF_OPEN) {
      ...
      // Check error rate threshold even on success - cumulative failures
      // may still exceed the threshold even after a successful call
  ...
  openCircuit(reason) {
    ...
    logAudit(AUDIT_EVENT_TYPES.CIRCUIT_OPENED, {
      circuit: this.name,
      reason,
      consecutiveFailures: this.consecutiveFailures,
      totalFailures: this.failures.length,
  ...
  closeCircuit() {
    ...
    logAudit(AUDIT_EVENT_TYPES.CIRCUIT_CLOSED, {
      circuit: this.name,
      previousState,
   ...
   * Called when the circuit is open and a fallback value/function is used
   */
  recordFallback() {
    this.stats.totalFallbacks++;
    this.stats.lastFallbackTime = Date.now();

    logger.debug(`Circuit '${this.name}' fallback used`);

    // Log audit event for fallback usage (non-blocking)
    logAudit(AUDIT_EVENT_TYPES.CIRCUIT_FALLBACK_USED, {
      circuit: this.name,
      state: this.state,
 ...
 *
 * Fallback Support:
 * - When the circuit is OPEN, instead of throwing an error, you can provide:
 *   1. A static fallbackValue to return
 *   2. A fallbackFn to execute for dynamic fallback behavior
 * - This enables graceful service degradation (e.g., return cached data, 
 *   default values, or reduced functionality instead of complete failure)
 *
 * @param {string} circuitName - Name of the circuit to use
 * @param {Function} fn - Async function to execute (the primary operation)
 ...
 * @param {*} options.fallbackValue - Static value to return when circuit is open (if throwOnOpen is false)
 * @param {Function} options.fallbackFn - Async function to execute as fallback when circuit is open
 * @returns {Promise<Object>} - Result object with success, result, circuitState, and fallback info
 * @throws {CircuitBreakerOpenError} - If circuit is open and throwOnOpen is true and no fallback provided
 * 
 * @example
 * // Basic usage with fallback value
 ...
 * );
 * 
 * // With fallback function for dynamic fallback
...
async function executeWithCircuit(circuitName, fn, options = {}) {
  ...
  // Circuit is OPEN - handle fallback or error
  if (!canExecute.allowed) {
    // Log fallback usage if we're going to use a fallback
    if (fallbackFn || fallbackValue !== undefined) {
    ...
    if (fallbackFn && typeof fallbackFn === 'function') {
      try {
        ...
        logger.debug(`Circuit '${circuitName}' fallback function executed successfully`);
        return { 
          success: true, 
        ...
        logger.warn(`Circuit '${circuitName}' fallback function failed: ${fallbackError.message}`);
        // Fall through to throw error or return fallback value
      }
    }

    // Return static fallback value if provided
    ...
    // No fallback available - throw or return error
    ...
    return { success: false, error, circuitOpen: true, circuitState: circuit.state };
  }

  // Circuit is CLOSED or HALF_OPEN - execute primary function
  try {
