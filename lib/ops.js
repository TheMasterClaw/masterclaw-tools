...
async function getServiceHealth() {
  ...
  for (const svc of services) {
    try {
      ...
      // Use secure HTTP client with private IP allowance for internal health checks
...
async function getRecentErrors() {
  ...
  try {
    ...
    const queryUrl = `${lokiUrl}/loki/api/v1/query_range?query=` +
      encodeURIComponent('{job="docker"} |= "error" or "ERROR" or "Error" or "CRITICAL"') +
      '&limit=10&start=' + (Date.now() - 3600000) + '000000'; // Last hour
    
    // Use secure HTTP client with private IP allowance for internal Loki
    const response = await httpClient.get(
      queryUrl,
      httpClient.allowPrivateIPs({
        timeout: 5000,
...
async function getCostStatus() {
  ...
  try {
    // Use secure HTTP client with private IP allowance for internal Core API
    const response = await httpClient.get(
      `${CORE_URL}/v1/costs`,
      httpClient.allowPrivateIPs({
        timeout: 5000,
