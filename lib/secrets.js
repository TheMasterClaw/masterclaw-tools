...
async function getDataEncryptionKey() {
  ...
  try {
    ...
    if (await fs.pathExists(KEY_FILE)) {
      const wrappedKey = await fs.readFile(KEY_FILE, 'utf-8');
  ...
  await fs.writeFile(KEY_FILE, wrappedKey, { mode: SECURE_FILE_MODE });
...
async function loadSecrets() {
  ...
  try {
    const fileData = await fs.readJson(SECRETS_FILE);
    ...
    if (needsMigration) {
      await fs.writeJson(SECRETS_FILE, migratedSecrets, { spaces: 2 });
      await fs.chmod(SECRETS_FILE, SECURE_FILE_MODE);
...
async function saveSecrets(secrets) {
  ...
  await fs.writeJson(SECRETS_FILE, encryptedSecrets, { spaces: 2 });
