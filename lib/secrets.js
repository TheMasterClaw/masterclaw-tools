...
// Encryption key file path (stores the wrapped/encrypted key)
const KEY_FILE = path.join(CONFIG_DIR, '.secrets.key');
...
/**
 * Derives a system-specific encryption key using OS-level entropy
 * This provides transparent encryption without requiring user passwords
 * The key is derived from system-specific data that persists across reboots
 * but is unique to this user/system combination
 *
 * Security note: This protects against:
 * - Casual snooping of secrets files
 * - Secrets exposure in backups
 * - Accidental file sharing
 *
 * This does NOT protect against:
 * - Root user on the same system (they can access memory/kernel)
 * - Determined attacker with system access
 *
 * For higher security, use external secret providers (future enhancement)
 */
function deriveSystemKey() {
  // Combine multiple system-specific sources for entropy
  // This creates a key that is:
  // 1. Unique to this user+system combination
  // 2. Consistent across process restarts
  // 3. Not easily guessable by other users
  const entropy = [
    os.userInfo().uid.toString(),
    os.userInfo().gid.toString(),
    os.homedir(),
    os.hostname(),
    os.platform(),
    // Machine-specific identifier (if available)
    process.env.MACHINE_ID || os.machine?.() || 'default',
  ].join('|');

  // Use PBKDF2-like derivation with high iteration count
  let key = crypto.createHmac(KEY_DERIVATION_ALGORITHM, 'MasterClawSecretsV1')
    .update(entropy)
    .digest();

  // Multiple iterations for added computational cost
  for (let i = 0; i < KEY_ITERATIONS / 1000; i++) {
    key = crypto.createHmac(KEY_DERIVATION_ALGORITHM, key)
      .update(entropy)
      .digest();
  }
...
/**
 * Generates a random encryption key for secrets
 * This key is itself encrypted with the system-derived key
 */
...
/**
 * Encrypts the data encryption key with the system key
 * Uses AES-256-GCM for authenticated encryption
 */
function wrapKey(dataKey, systemKey) {
  const iv = crypto.randomBytes(IV_LENGTH);
  const salt = crypto.randomBytes(SALT_LENGTH);

  // Derive a key-specific encryption key using the salt
  ...
  const cipher = crypto.createCipheriv(ENCRYPTION_ALGORITHM, keyEncryptionKey, iv);
  const encrypted = Buffer.concat([cipher.update(dataKey), cipher.final()]);
  const authTag = cipher.getAuthTag();

  // Format: salt(32) + iv(16) + authTag(16) + encryptedKey(32)
  return Buffer.concat([salt, iv, authTag, encrypted]).toString('base64');
}

/**
 * Unwraps (decrypts) the data encryption key
 * Returns null if decryption fails (tampering detected or wrong system)
 */
function unwrapKey(wrappedKey, systemKey) {
  try {
    ...
    // Derive the same key-specific encryption key
    const keyEncryptionKey = crypto.pbkdf2Sync(
      systemKey,
      salt,
      10000,
    ...
    const decipher = crypto.createDecipheriv(ENCRYPTION_ALGORITHM, keyEncryptionKey, iv);
    decipher.setAuthTag(authTag);

    const decrypted = Buffer.concat([decipher.update(encrypted), decipher.final()]);
    return decrypted;
  } catch (err) {
    // Decryption failed - possible tampering or wrong system
    return null;
  }
}

/**
 * Gets or creates the data encryption key
 * The key is stored wrapped (encrypted) and only unwrapped in memory
 */
async function getDataEncryptionKey() {
  const systemKey = deriveSystemKey();

  // Use module.exports to allow test overrides
  const keyFile = module.exports.KEY_FILE || KEY_FILE;

  try {
    // Check if we have a stored wrapped key
    if (await fs.pathExists(keyFile)) {
      const wrappedKey = await fs.readFile(keyFile, 'utf-8');
      const dataKey = unwrapKey(wrappedKey, systemKey);

      if (dataKey) {
        return dataKey;
      }
      // If unwrapping failed, key file may be corrupted or from different system
      // Generate a new key (secrets will need to be re-entered)
      console.warn(chalk.yellow('[Secrets] Key file invalid or from different system. Generating new encryption key.'));
    }
  } catch (err) {
    // Key file read failed
    console.warn(chalk.yellow('[Secrets] Could not read key file. Generating new encryption key.'));
  }

  // Generate new key and wrap it
  const dataKey = generateEncryptionKey();
  const wrappedKey = wrapKey(dataKey, systemKey);

  await fs.writeFile(keyFile, wrappedKey, { mode: SECURE_FILE_MODE });
 ...
 * Returns encrypted data as base64 string with embedded IV and auth tag
 */
...
/**
 * Checks if the secrets file is using the new encrypted format
 * Encrypted secrets have a special marker prefix
 */
function isEncryptedValue(value) {
  if (typeof value !== 'string') return false;
  // Encrypted values are base64 strings starting with the encrypted marker
  // We detect this by checking if the value looks like our encrypted format
  try {
...
async function ensureSecretsFile() {
  ...
  // Use module.exports to allow test overrides
  const secretsFile = module.exports.SECRETS_FILE || SECRETS_FILE;

  if (!(await fs.pathExists(secretsFile))) {
    await fs.writeJson(secretsFile, {}, { spaces: 2 });
  }

  // Ensure secure permissions
  try {
    await fs.chmod(secretsFile, SECURE_FILE_MODE);
  } catch (err) {
...
/**
 * Load secrets from file (with automatic decryption)
 * Transparently migrates plaintext secrets to encrypted on first load
 */
async function loadSecrets() {
  ...
  try {
    const fileData = await fs.readJson(secretsFile);
    const secrets = fileData || {};

    // Get the data encryption key
    const dataKey = await getDataEncryptionKey();

    // Check for plaintext secrets that need migration
    let needsMigration = false;
    const migratedSecrets = {};

    for (const [key, data] of Object.entries(secrets)) {
      if (data && typeof data === 'object' && data.value) {
        // Check if this is an old plaintext secret that needs encryption
        if (!isEncryptedValue(data.value)) {
          // Encrypt the plaintext value
          const encryptedValue = encryptSecret(data.value, dataKey);
          migratedSecrets[key] = {
            ...data,
    ...
    // Save migrated secrets if needed
    if (needsMigration) {
      await fs.writeJson(secretsFile, migratedSecrets, { spaces: 2 });
      await fs.chmod(secretsFile, SECURE_FILE_MODE);

      // Log the migration event
      await logAudit(AuditEventType.CONFIG_WRITE, {
        action: 'secrets_encrypted',
        message: 'Migrated plaintext secrets to encrypted storage',
        count: Object.keys(migratedSecrets).length,
      });
    }
...
async function saveSecrets(secrets) {
  ...
  // Ensure all secret values are encrypted before saving
  const dataKey = await getDataEncryptionKey();
  ...
  for (const [key, data] of Object.entries(secrets)) {
    if (data && typeof data === 'object' && data.value) {
      ...
      const valueToStore = isEncryptedValue(data.value)
        ? data.value
        : encryptSecret(data.value, dataKey);
      ...
      encryptedSecrets[key] = data;
    }
  }

  await fs.writeJson(secretsFile, encryptedSecrets, { spaces: 2 });
...
async function setSecret(key, value, options = {}) {
  ...
  await logAudit(oldValue ? AuditEventType.CONFIG_WRITE : AuditEventType.CONFIG_WRITE, {
...
async function getSecret(key) {
  ...
  if (isEncryptedValue(value)) {
    const dataKey = await getDataEncryptionKey();
    const decrypted = decryptSecret(value, dataKey);

    if (decrypted === null) {
      // Decryption failed - possible tampering
      console.error(chalk.red(`[Secrets] Failed to decrypt '${key}'. Data may be corrupted or tampered with.`));
      return null;
    }
  ...
  // Plaintext value (should not happen after migration, but handle gracefully)
  return value;
}
