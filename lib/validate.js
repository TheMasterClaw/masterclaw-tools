...
/**
 * Private/internal IP ranges that should not be used in production domains.
 * These are SSRF attack vectors where an attacker could use a domain that
 * resolves to an internal IP to bypass security controls.
 */
...
/**
 * Internal hostnames that could indicate SSRF or misconfiguration.
 * These should not be used as public-facing domains.
 */
const INTERNAL_HOSTNAME_INDICATORS = [
  'localhost',
...
/**
 * Suspicious domain patterns that could indicate DNS rebinding attacks
 * or other SSRF-related misconfigurations.
 */
const SUSPICIOUS_DOMAIN_PATTERNS = [
  /^\d+\.\d+\.\d+\.\d+/,              // IP addresses as domain
  /\.local$/,                            // mDNS/Bonjour domains
  /\.internal$/,                         // Internal domains
  /\.lan$/,                              // Local network domains
  /\.home$/i,                            // Home network domains
  /[^a-zA-Z0-9.-]/,                      // Invalid characters in domain
  /\.\d+$/,                              // Domain ending in number (suspicious)
  /(0x[0-9a-f]+|\\d{1,3}\\.\\d{1,3})/i,  // Hex/octal IP encoding attempts
];

/**
 * Validates that a domain does not contain SSRF attack vectors.
 * Checks for private IPs, internal hostnames, and suspicious patterns.
 *
 * @param {string} domain - Domain to validate
 * @returns {Object} - Validation result with warnings array
 */
function validateDomainSSRFProtection(domain) {
  const warnings = [];

  if (!domain || typeof domain !== 'string') {
    return { valid: false, warnings: ['Domain is required'] };
  }

  const lowerDomain = domain.toLowerCase().trim();

  // Check for IP address patterns (direct IP as domain)
  for (const pattern of PRIVATE_IP_PATTERNS) {
    if (pattern.test(lowerDomain)) {
      warnings.push(`Domain '${domain}' resolves to a private/internal IP address - potential SSRF vector`);
      break;
    }
  }

  // Check for internal hostnames
  for (const indicator of INTERNAL_HOSTNAME_INDICATORS) {
    if (lowerDomain === indicator ||
        lowerDomain.includes(`.${indicator}.`) ||
        lowerDomain.endsWith(`.${indicator}`)) {
      warnings.push(`Domain '${domain}' appears to be an internal hostname - not suitable for production`);
      break;
    }
  }
  ...
  for (const pattern of SUSPICIOUS_DOMAIN_PATTERNS) {
    if (pattern.test(lowerDomain)) {
      if (/^\d+\.\d+\.\d+\.\d+/.test(lowerDomain)) {
        warnings.push(`Domain '${domain}' is an IP address - use a proper domain name for SSL certificates`);
      } else if (/\.local$/.test(lowerDomain)) {
        warnings.push(`Domain '${domain}' uses .local TLD (mDNS) - SSL certificates will not work`);
      } else if (/\.internal$|\.lan$|\.home$/i.test(lowerDomain)) {
        warnings.push(`Domain '${domain}' appears to be an internal network domain - not publicly resolvable`);
      } else if (/[^a-zA-Z0-9.-]/.test(lowerDomain)) {
        warnings.push(`Domain '${domain}' contains invalid characters`);
      } else {
        warnings.push(`Domain '${domain}' has a suspicious pattern - verify this is intentional`);
      }
      break;
    }
  }

  // Check for potential DNS rebinding indicators
  if (lowerDomain.split('.').some(part => {
    // Check for numeric-only parts that could be IP octets
    const num = parseInt(part, 10);
    return !isNaN(num) && num >= 0 && num <= 255 && String(num) === part;
  }) && !/^\d+\.\d+\.\d+\.\d+/.test(lowerDomain)) {
    warnings.push(`Domain '${domain}' contains numeric components that could indicate DNS rebinding attempt`);
  }

  return {
    valid: warnings.length === 0,
    warnings,
    isInternal: warnings.some(w => w.includes('internal') || w.includes('private')),
    isSSRFVector: warnings.some(w => w.includes('SSRF')),
  };
}
...
/**
 * Default timeout for port availability check (5 seconds)
 * Prevents indefinite hanging if the network stack doesn't respond
 */
const PORT_CHECK_TIMEOUT_MS = 5000;

/**
 * Minimum valid port number (well-known ports require root on Unix)
 */
const MIN_VALID_PORT = 1;

/**
 * Maximum valid port number (TCP/UDP port range)
 */
...
/**
 * Check if a port is available with timeout protection
 * Security: Prevents indefinite hanging with timeout
 * Security: Validates port number to prevent OS-level issues
 * @param {number} port - Port to check
 * @param {number} timeoutMs - Timeout in milliseconds (default: 5000ms)
 * @returns {Promise<boolean>} - True if port is available, false if in use or invalid
 */
async function isPortAvailable(port, timeoutMs = PORT_CHECK_TIMEOUT_MS) {
  ...
  return new Promise((resolve) => {
    ...
    timeoutId = setTimeout(() => {
      if (!resolved) {
        ...
        try {
          server.close(() => safeResolve(false));
        } catch {
          safeResolve(false);
        }
      }
    ...
    server.once('error', (err) => {
      ...
      // Any error means port is not available or we can't use it
      safeResolve(false);
    });

    server.once('listening', () => {
      server.removeAllListeners();
      // Close the server and resolve based on close success
      server.close((err) => {
        if (timeoutId) {
          clearTimeout(timeoutId);
          timeoutId = null;
        }
        // If close succeeded without error, port is available
        safeResolve(!err);
      });
    });

    // Handle edge case where listen() throws synchronously
    try {
      server.listen(port);
    } catch (err) {
      safeResolve(false);
...
async function validate(options = {}) {
  ...
  if (!await fs.pathExists(envPath)) {
    ...
    // Validate DOMAIN format and SSRF protection
    if (envVars.DOMAIN) {
      ...
      const ssrfCheck = validateDomainSSRFProtection(envVars.DOMAIN);
      if (ssrfCheck.warnings.length > 0) {
        for (const warning of ssrfCheck.warnings) {
          results.warnings.push({
            type: ValidationErrorType.WARNING,
            message: warning,
          });
        ...
        // Add specific security guidance for SSRF vectors
        if (ssrfCheck.isSSRFVector) {
          results.info.push({
            type: ValidationErrorType.INFO,
            message: 'SSRF protection: Use a public domain with proper DNS resolution for production',
          });
        }
      }
    }
...
module.exports = {
  ...
  MAX_ENV_FILE_LINES,
  isDangerousEnvKey,
  // Port checking security exports
