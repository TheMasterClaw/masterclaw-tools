...
function generateEntrySignature(entry, key) {
  ...
  // Exclude any existing signature field to avoid circular issues
  const { _signature, ...entryToSign } = entry;
  
  const canonicalData = JSON.stringify(entryToSign, Object.keys(entryToSign).sort());
