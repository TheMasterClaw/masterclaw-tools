...
async function executeBackupVerify(options = {}) {
  ...
  // Log audit event
  await logAudit(AuditEventType.BACKUP_RESTORE, {
...
/**
 * Manage scheduled backup verification
 *
 * @param {string} action - Action to perform (install, remove, status, run-now, log)
 * @param {Object} options - Schedule options
 * @returns {Promise<Object>} - Result
 */
async function manageScheduledVerification(action, options = {}) {
  const infraDir = await findInfraDir();

  if (!infraDir) {
    const error = new Error('MasterClaw infrastructure directory not found');
    error.code = 'INFRA_NOT_FOUND';
    throw error;
  }

  const scriptPath = path.join(infraDir, 'scripts', 'schedule-backup-verify.sh');

  if (!await fs.pathExists(scriptPath)) {
    const error = new Error(`Schedule script not found: ${scriptPath}`);
    error.code = 'SCRIPT_NOT_FOUND';
    throw error;
  }
  ...
  if (options.daily) args.push('--daily');
  if (options.weekly) args.push('--weekly');
  if (options.monthly) args.push('--monthly');
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        const error = new Error(`Schedule command failed with exit code ${code}`);
...
backupVerify
  ...
  .action(async (options) => {
    // If no options specified, default to --latest
    if (!options.file && !options.latest && !options.all) {
      options.latest = true;
    }
    ...
    try {
      await executeBackupVerify(options);
...
backupVerify
  .command('schedule')
  .description('Schedule automated backup verification')
  .argument('<action>', 'Action: install, remove, status, run-now, log')
  .option('--daily', 'Schedule daily verification (with install)')
  .option('--weekly', 'Schedule weekly verification (default, with install)')
  .option('--monthly', 'Schedule monthly verification (with install)')
  .option('--webhook <url>', 'Webhook URL for failure notifications')
  .option('--email <address>', 'Email address for failure notifications')
  .action(async (action, options) => {
    const validActions = ['install', 'remove', 'status', 'run-now', 'log'];

    if (!validActions.includes(action)) {
      console.error(chalk.red(`❌ Invalid action: ${action}`));
      console.error(chalk.gray(`   Valid actions: ${validActions.join(', ')}`));
      process.exit(1);
    }

    console.log(chalk.blue(`🗓️  Backup Verification Schedule: ${action}\n`));

    try {
      await manageScheduledVerification(action, options);
    } catch (err) {
      if (err.code === 'INFRA_NOT_FOUND') {
        console.error(chalk.red(`❌ ${err.message}`));
        console.error(chalk.gray('   Set MASTERCLAW_INFRA environment variable or run from infrastructure directory'));
      } else {
...
module.exports.manageScheduledVerification = manageScheduledVerification;
