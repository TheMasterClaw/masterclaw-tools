...
/**
 * Monitor command - manage automated SSL certificate monitoring
 */
ssl
  .command('monitor')
  .description('Manage automated SSL certificate expiration monitoring')
  .option('--install', 'Install automated SSL monitoring')
  .option('--remove', 'Remove SSL monitoring')
  .option('--status', 'Check monitoring status')
  .option('--run-now', 'Run SSL check immediately')
  .option('--logs', 'View monitoring logs')
  .option('--test-notify', 'Test notification settings')
  .option('--schedule <type>', 'Schedule type: hourly, daily (default), weekly', 'daily')
  .option('--warning <days>', 'Warning threshold in days', '14')
  .option('--critical <days>', 'Critical threshold in days', '7')
  .option('--webhook <url>', 'Webhook URL for notifications')
  .option('--email <address>', 'Email address for notifications')
  .option('--auto-renew', 'Enable automatic renewal for critical certificates')
  .action(async (options) => {
    const infraDir = findInfraDir();
    ...
    const scriptPath = path.join(infraDir, 'scripts', 'schedule-ssl-check.sh');

    if (!fs.existsSync(scriptPath)) {
      console.error(chalk.red('❌ SSL monitoring script not found'));
      console.log(chalk.gray(`Expected at: ${scriptPath}`));
      console.log(chalk.gray('Ensure you have the latest infrastructure scripts.'));
      process.exit(1);
    }

    // Determine which action to take
    let command = null;
    if (options.install) command = 'install';
    else if (options.remove) command = 'remove';
    else if (options.status) command = 'status';
    else if (options.runNow) command = 'run-now';
    else if (options.logs) command = 'log';
    else if (options.testNotify) command = 'notify-test';
    ...
    if (options.install) {
      // Add schedule option
      if (options.schedule) {
        args.push(`--${options.schedule}`);
      }

      // Add thresholds
      args.push('--warning', options.warning);
      args.push('--critical', options.critical);
