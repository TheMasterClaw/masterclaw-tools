...
async function getDirectorySize(dirPath) {
  try {
    ...
    const output = execSync(`du -sk "${dirPath}" 2>/dev/null || echo "0 ${dirPath}"`, {
      encoding: 'utf8',
      timeout: 30000,
    });
...
async function getDirectoryBreakdown(dirPath, maxDepth = 1) {
  try {
    ...
    const output = execSync(`du -k --max-depth=${maxDepth} "${dirPath}" 2>/dev/null || echo ""`, {
      encoding: 'utf8',
      timeout: 30000,
    });
...
async function findInfraDir() {
  ...
  if (process.env.MASTERCLAW_INFRA_DIR) {
    return process.env.MASTERCLAW_INFRA_DIR;
  }
