...
backup
  ...
  .action(async (options) => {
    ...
    await logAuditEvent('backup_start', {
    ...
    try {
      const result = await runBackup(infraDir, {
        retentionDays: options.retentionDays,
        quiet: options.quiet,
      });
      ...
      await logAuditEvent('backup_complete', { success: true });
      ...
      await logAuditEvent('backup_error', { error: err.message });
      process.exit(1);
    }
  });
...
backup
  ...
  .action(async (options) => {
    ...
    await logAuditEvent('backup_cleanup', {
      retentionDays,
      deleted,
      failed,
